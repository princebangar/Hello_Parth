...
export const getCategoryListCacheKey = (zoneId) =>
  `helloparth_cat_list_zone_${zoneId || "all"}`;
...
export const getCategoryRestaurantCacheKeys = (
  ...
  const keys = new Set();
  if (raw) keys.add(`helloparth_cat_${raw}_zone_${zone}`);
  ...
  if (matched) {
    ...
    if (slug) keys.add(`helloparth_cat_${slug}_zone_${zone}`);
    if (id && id !== "all") keys.add(`helloparth_cat_${id}_zone_${zone}`);
  }
